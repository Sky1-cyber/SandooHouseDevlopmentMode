...
public async Task<IActionResult> EditOrder([FromBody] EditOrderRequest request)
{
    try
    {
        ...
        try
        {
            ...
            if (request.CashReceived < totalAmount && request.CashReceived > 0)
            {
                ...
                Console.WriteLine($"Warning: Cash received (${request.CashReceived}) is less than total (${totalAmount})");
            }
            ...
            existingOrder.ChangeAmount = request.CashReceived >= totalAmount ? request.CashReceived - totalAmount : 0;
    ...
    catch (DbUpdateException dbEx)
    {
        ...
        return StatusCode(500, new { error = "A database error occurred while updating the order. Please check your data and try again." });
    }
    catch (Exception ex)
    {
